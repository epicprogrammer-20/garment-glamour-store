  ...
  const fetchSaleProducts = async () => {
    try {
      const { data, error } = await supabase
        .from('sale_products')
        .select(`*, products (id, name, image, category, description, sizes)`)
        .eq('is_active', true)
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background">
        <Header />
        <div className="max-w-7xl mx-auto px-4 py-16"><div className="text-center"><p>Loading sale items...</p></div></div>
        <Footer />
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            <div className="bg-gradient-to-br from-purple-600 to-purple-800 text-white p-8 rounded-xl text-center transform hover:scale-105 transition-transform">
              ...
              <button className="bg-white text-purple-600 px-6 py-2 rounded-full font-semibold mt-4 hover:bg-gray-100 transition-colors">SHOP NOW</button>
            </div>
            <div className="bg-gradient-to-br from-pink-600 to-pink-800 text-white p-8 rounded-xl text-center transform hover:scale-105 transition-transform">
              ...
              <button className="bg-white text-pink-600 px-6 py-2 rounded-full font-semibold mt-4 hover:bg-gray-100 transition-colors">BUY NOW</button>
            </div>
            <div className="bg-gradient-to-br from-green-600 to-green-800 text-white p-8 rounded-xl text-center transform hover:scale-105 transition-transform">
              ...
              <p className="text-lg">Orders Over {formatPrice(100)}</p>
              <button className="bg-white text-green-600 px-6 py-2 rounded-full font-semibold mt-4 hover:bg-gray-100 transition-colors">SHOP FREE</button>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-center space-x-8">
            <div className="flex items-center space-x-3">
              <div className="bg-red-600 p-3 rounded-full"><ShoppingBag size={24} /></div>
              <div><h3 className="text-xl font-bold text-red-400">FLASH SALE</h3><p className="text-sm">Ends in 24 hours</p></div>
            </div>
            <div className="hidden md:block h-12 w-px bg-gray-600"></div>
            <div className="text-center"><p className="text-3xl font-bold text-red-400">UP TO 50% OFF</p><p className="text-sm">Everything Must Go!</p></div>
            <div className="hidden md:block h-12 w-px bg-gray-600"></div>
            <button className="bg-red-600 hover:bg-red-700 px-8 py-3 rounded-lg font-semibold transition-colors">SHOP THE SALE</button>
          </div>
        </div>
      </section>
      ...
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-4xl font-bold text-foreground mb-4">Sale Items</h2>
            <p className="text-muted-foreground max-w-2xl mx-auto">Don't miss out on these incredible deals! Limited quantities available.</p>
          </div>
          ...
          {saleProducts.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {saleProducts.map((product) => (
                <div key={product.id} className="relative">
                  <div className="absolute top-4 left-4 z-10 bg-red-600 text-white px-3 py-1 rounded-full text-sm font-bold">{product.discountPercentage}% OFF</div>
            ...
            <div className="text-center py-16">
              ...
              <p className="text-muted-foreground mt-2">Check back soon for amazing deals!</p>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="bg-gradient-to-r from-red-600 to-pink-600 text-white py-16">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <button className="bg-white text-red-600 px-12 py-4 rounded-full text-xl font-bold hover:bg-gray-100 transition-colors transform hover:scale-105">SHOP SALE NOW</button>
        </div>
      </section>
